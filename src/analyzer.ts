...
export class ASOAnalyzer {
   ...
   */
  static async analyzeCompetitors(keyword: string, apps: AppInfo[]) {
    const competitors = await Promise.all(
      apps.map(async app => {
        const keywords = await KeywordAnalyzer.extractKeywords(
  ...
  static calculateInstallsScore(apps: AppInfo[], store: StoreType) {
    const getMetric = (app: AppInfo) =>
      store === 'gplay' ? (app.minInstalls || 0) : (app.reviews || 0);
  ...
  static calculateAgeScore(apps: AppInfo[]) {
    const daysSinceUpdated = apps.map(app =>
      KeywordAnalyzer.getDaysSince(app.updated || 0)
    );
   ...
   */
  static async calculateSuggestScore(keyword: string, aso: ASO) {
    ...
    const calculateGPlaySuggestScore = async (length = 1): Promise<{
      length?: number;
      index?: number;
      score: number;
    }> => {
   ...
   */
  static async calculateRankedScore(apps: AppInfo[], aso: ASO) {
    try {
      ...
      const avgRank = R.mean(rankings.filter((rank): rank is number => rank !== undefined));
   ...
   */
  static async getAppsByStrategy(options: SuggestOptions, aso: ASO): Promise<AppInfo[]> {
    switch (options.strategy) {
      ...
      case 'competition': {
        ...
        const searches = await Promise.all(
          topKeywords.map(kw =>
            aso.search({ term: kw, num: 10, fullDetail: true })
          )
        );
      ...
      case 'keywords':
        if (!options.keywords?.length) return [];
        return this.getAppsFromKeywords(options.keywords, aso);

      case 'arbitrary':
        if (!options.apps?.length) return [];
        return Promise.all(
          options.apps.map(id => aso.getAppInfo(id))
        );
   ...
   */
  private static async getAppsFromKeywords(
    keywords: string[],
    aso: ASO
  ): Promise<AppInfo[]> {
    const searches = await Promise.all(
   ...
   */
  static calculateMarketOpportunity(
    saturation: number,
    difficulty: number,
    traffic: number
  ): number {
    return ScoreCalculator.aggregate(
      [4, 3, 3],
      [
        10 - saturation,
        10 - difficulty,
   ...
   */
  static calculateMarketSaturation(
    searchResults: SearchResult[],
    minInstalls: number = 10000
  ): number {
    const totalApps = searchResults.length;
    if (totalApps === 0) return 0;

    const saturatedApps = searchResults.filter(
      app => (app.minInstalls || 0) >= minInstalls
    ).length;
   ...
   */
  static generateKeywordCombinations(
    keywords: string[],
  ...
  static calculateKeywordRelevancy(keyword: string): number {
    ...
    const lengthScore = ScoreCalculator.score(
      1,
      25,
      length > 20 ? length * 0.8 : length
    );
